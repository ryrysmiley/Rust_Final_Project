...
// game of life start game CLI interface
// pub fn start_game() {
...
pub fn send_final_grids(gamemode: i32, row: i32, width: i32) -> Vec<Vec<Vec<char>>> {
    let mut grid = GoLGrid::new(width as usize, row as usize);
    //populate the grid
    grid.randomize();
    //run generations
    while grid.get_gen_num() < 1000 && !grid.check_match(){
